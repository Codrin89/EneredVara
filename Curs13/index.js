(function() {
	// select all hover boxes from the page
	...
	for(var i = 0; i < boxesHover.length; i++) {
		boxesHover[i].addEventListener('click', function() {
			...
			if(boxesHoverSelected.length > 0) {
				...
				document.getElementsByClassName('info2')[0].innerHTML = boxesHoverSelected.length + ' slides (' + totalSize.toFixed(1) + 'mb) selected for download'
			}
