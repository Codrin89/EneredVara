(function(){
	...
	for (var i = 0 ; i < boxesHover.length ; i++)	{
        boxesHover[i].addEventListener("click", function() {
         ...
         if(boxesHoverSelected.length > 0 && document
         	.getElementsByClassName("downloadBox")[0].classList.contains("displayNone")){
         	document.getElementsByClassName("downloadBox")[0].classList
         	   .remove("displayNone");
         ...
         if(boxesHoverSelected.length == 0){
          ...
          if(boxesHoverSelected.length > 0) {
          	...
          	document.getElementsByClassName("info")[0].innerHTML = 
          	    boxesHoverSelected.length + "slides ("+ totalSize.toFixed(1) + "mb) selected for download";
          }

         alert(boxesHoverSelected.length + "slide size="+ totalSize.toFixed(1));
