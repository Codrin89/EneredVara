(function() {
	...
	for(var i = 0 ; i < boxesHover.length ; i++) {
		boxesHover[i].addEventListener('click', function() {
			...
			if(boxesHoverSelected.length > 0 && document.getElementsByClassName('downnloadbox')[0].classList.contains('displayNone')) {
				document.getElementsByClassName('downnloadbox')[0].classList.remove('displayNone');
			} 	
			if(boxesHoverSelected.length == 0) {
				document.getElementsByClassName('downnloadbox')[0].classList.add('displayNone');
			}
			if(boxesHoverSelected.length > 0) {
				var totalSize = 0;
